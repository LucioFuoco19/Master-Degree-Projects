...
void setGhost(int tempo){ //gestisco aggressività ghost
	if(tempo==50){
			disable_timer(1);
			reset_timer(1);
			init_timer(1,0x00aba950);//450 ms * 25 Mhz
	...
	if(tempo==20){
			disable_timer(1);
			reset_timer(1);
			init_timer(1,0x005f5e10);//250 ms * 25 Mhz
			if(morto!=1)enable_timer(1);
	} if(tempo==10){
			disable_timer(1);
			reset_timer(1);
			init_timer(1,0x004c4b40); //200 ms * 25 Mhz
			if(morto!=1)enable_timer(1);
	}
}
...
int dx[]={-1,1,0,0};
	...
	for(int i=0;i<4;i++){
			...
			int newY=g->y+dy[i];
			if((newX!=g->predX)||(newY!=g->predY)){
				if((g->x==11)&&(g->y==0)){//caso teleport
					if(newY==-1)newY=23;
				}else if((g->x==11)&&(g->y==23)){
						if(newY==24)newY=0;
				}
			if(checkMovement(newX,newY)){
				int distanza=abs(xpac-newX)+abs(ypac-newY);
