...
describe("UserRepository: mocked database", () => {
  ...
  it("find all users", async () => {
    ...
    foundUsers[0].username = "john";
    foundUsers[0].password = "pass123";
    foundUsers[0].type = UserType.Admin;
    foundUsers[1].username = "jane";
    foundUsers[1].password = "pass456";
    foundUsers[1].type = UserType.Operator;

    mockFind.mockResolvedValue(foundUsers);

    const result = await repo.getAllUsers();
    expect(result).toHaveLength(2);
    expect(result[0]).toBe(foundUsers[0]);
    expect(result[1]).toBe(foundUsers[1]);
  });
