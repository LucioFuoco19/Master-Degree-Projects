...
pub fn show(app: &mut App, ctx: &egui::Context, idx: usize) {
    let Some(group) = app.groups.get(idx) else {
        egui::CentralPanel::default().show(ctx, |ui| { ui.label("Group not found"); if ui.button("Back").clicked() { app.nav(View::ChatList); }});
        return;
    };
    ...
    egui::TopBottomPanel::top("chat_top_panel")
        ...
        .show(ctx, |ui| {
            ui.horizontal(|ui| {
                ...
                ui.add_space(10.0);
                ui.label(RichText::new("👥").size(26.0)); // Balanced icon

                ui.vertical(|ui| {
                    // *** FIX 1: Intermediate size (20.0) ***
                    ui.label(RichText::new(&group_name).strong().size(20.0));

                    // Subtitle with list of names
                ...
                ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                    ...
                    if ui.add(egui::Button::new(RichText::new("ℹ").size(20.0)).fill(info_btn_fill).rounding(Rounding::same(8.0))).on_hover_text("Group Info & Members").clicked() {
                        app.show_group_info = !app.show_group_info;
                        // Clean up the search if it closes/opens in order
    ...
    if app.show_group_info {
        egui::SidePanel::right("group_info_panel")
            ...
            .show(ctx, |ui| {
                ui.horizontal(|ui| {
                    ui.heading(RichText::new("Group Details").strong().size(18.0));
                    ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                ...
                ui.label(RichText::new("Add Participant").strong());
                ui.add_space(5.0);
                ...
                if !app.search_results.is_empty() {
                    ui.add_space(5.0);
                    ui.label(RichText::new("Search Results:").small().weak());
                                ...
                                ui.vertical(|ui| {
                                    ...
                                    for user in results {
                                        ...
                                        ui.horizontal(|ui| {
                                            ...
                                            // Right layout: Show either "Already Member" or the "+" key
                                            ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                                                if is_already_member {
                                                    ui.label(RichText::new("Already Member").small().italics().color(Color32::from_gray(120)));
                                                } else {
                                                    if ui.add(egui::Button::new(RichText::new("➕").color(Color32::WHITE))
                                                        .fill(Color32::from_rgb(0, 100, 200))) // Dark Blue
                                                        .on_hover_text("Add to Group")
                                                        .clicked()
                                                    {
                    ...
                    ui.vertical_centered(|ui| {
                        ui.label(RichText::new("No users found").italics().color(Color32::from_gray(120)));
                    });
                }
                ...
                // - CURRENT MEMBER LIST ---
                ui.label(RichText::new(format!("Group members ({})", app.current_group_members.len())).strong());
                ui.add_space(5.0);
            ...
            egui::ScrollArea::vertical().stick_to_bottom(true).auto_shrink([false, false]).show(ui, |ui| {
                if app.current_messages.is_empty() {
                    ui.vertical_centered(|ui| {
                        ...
                        ui.label(RichText::new("Welcome to the group.").color(Color32::from_gray(150)));
                    });
                } else {
...
fn search_users_logic(app: &mut App, ctx: &egui::Context) {
    if let Some(token) = app.session_token {
        ...
        handle.spawn(async move {
            // We use the same search function used for private chats
...
fn add_member_logic(app: &mut App, ctx: &egui::Context, group_id: Uuid) {
    if let Some(token) = app.session_token {
        let tx = app.get_tx();
        app.is_loading = true;
        app.status = format!("Added {}", app.add_member_username).into();
        let addr = app.server_addr.clone();
...
fn format_timestamp(unix_time: i64) -> String {
    ...
    if let Ok(elapsed) = SystemTime::now().duration_since(timestamp) {
        ...
        if secs < 60 { return "hour".to_string(); } else if secs < 3600 { return format!("{} min ago", secs / 60); } else if secs < 86400 { return format!("{} hours ago", secs / 3600); }
    }
