    ...
    let Some(chat) = app.private_chats.get(idx) else {
        egui::CentralPanel::default().show(ctx, |ui| {
            ui.vertical_centered(|ui| {
                ...
                if ui.button("← Back to the list").clicked() { app.nav(View::ChatList); }
            });
        });
        return;
    };
            ...
            egui::Frame::none()
                ...
                .show(ui, |ui| {
                    ui.horizontal(|ui| {
                        ...
                        // *** FIX: We use "➡️" (Emoji) instead of complex unicode characters ***
            ...
            egui::ScrollArea::vertical().stick_to_bottom(true).auto_shrink([false, false]).show(ui, |ui| {
                if app.current_messages.is_empty() {
                    ui.vertical_centered(|ui| {
                        ...
                        ui.label(RichText::new("No message.").color(Color32::from_gray(150)));
                    });
                } else {
...
fn format_timestamp(unix_time: i64) -> String {
    ...
    if let Ok(elapsed) = SystemTime::now().duration_since(timestamp) {
        ...
        else if secs < 3600 { return format!("{} min ago", secs / 60); }
        else if secs < 86400 { return format!("{} hours ago", secs / 3600); }
    }
