...
pub fn show(app: &mut App, ctx: &egui::Context) {
    ...
    // Timer (Debounce) logic for search
    ...
    // - COMMON STYLES (Taken from Home/Login) ---
    let panel_fill = ctx.style().visuals.window_fill();
    // "Card" background: slightly lighter than the window background
    ...
    egui::SidePanel::left("chat_options_panel")
        .default_width(300.0)
        .resizable(false)
        .frame(egui::Frame::none().fill(panel_fill)) // Uniform base background
        .show(ctx, |ui| {
            ui.add_space(20.0);
            ui.vertical_centered(|ui| {
                ui.heading(RichText::new("Search & Create").strong().size(22.0));
            });
            ui.add_space(20.0);
            ...
            egui::Frame::none()
                ...
                .show(ui, |ui| {
                    ui.horizontal(|ui| {
                        ...
                        ui.label(RichText::new("Search users").strong().size(16.0));
                    });
                    ui.add_space(10.0);

                    let response = ui.add(
                        egui::TextEdit::singleline(&mut app.search_query)
                            .hint_text("Write username...")
                    ...
                    if !app.search_results.is_empty() {
                        ...
                        ui.separator();
                        ui.add_space(5.0);
                        ui.label(RichText::new("Results:").small().weak());

                        let results = app.search_results.clone();
                        egui::ScrollArea::vertical().id_source("search_results").max_height(200.0).show(ui, |ui| {
                            for user in results {
                                ui.add_space(4.0);
                                // "Card" for single result
                                egui::Frame::none()
                                    .fill(Color32::from_black_alpha(40)) // Darker for contrast
                                                ...
                                                if ui.add_enabled(!is_self, egui::Button::new("💬 Chat").small()).clicked() {
                                                    ...
                                                    if let Some(idx) = existing_idx {
                                                        ...
                                                        if let Some(token) = app.session_token {
                                                            ...
                                                            let tx = app.get_tx();
                                                            app.is_loading = true;
                                                            app.status = format!("Starting chat with {}...", other_username);
                                                            let addr = app.server_addr.clone();
                                                            let handle = app.rt.as_ref().unwrap().handle().clone();
                                                            let ctx2 = ctx.clone();
                                                            handle.spawn(async move {
                        ...
                        ui.add_space(10.0);
                        ui.label(RichText::new("No users found.").small().weak());
                    }
                });
            ...
            egui::Frame::none()
                ...
                .show(ui, |ui| {
                    ui.horizontal(|ui| {
                        ui.label(RichText::new("➕").size(18.0));
                        ui.label(RichText::new("Create Group").strong().size(16.0));
                    });
                    ui.add_space(10.0);
                    ...
                    // Login Style Button (Blue)
                    let create_btn = egui::Button::new(RichText::new("Create Group").strong().color(Color32::WHITE))
                    ...
                    if ui.add_enabled(can_create, create_btn).clicked() {
                        if let Some(token) = app.session_token {
                            let tx = app.get_tx();
                            app.is_loading = true;
                            app.status = format!("Group creation {}...", app.new_group_name);
                            let addr = app.server_addr.clone();
            ...
            // *** EDIT: Removed "Update Lists" button below ***
        });

    // --- PANNELLO CENTRALE (Lista) ---
    egui::CentralPanel::default().show(ctx, |ui| {
        ui.add_space(20.0);

        // Intestazione
        ui.horizontal(|ui| {
            ui.heading(RichText::new("Your messages").size(28.0).strong());
            if app.is_loading {
                ui.add_space(10.0);
                ui.spinner();
            }
        ...
        ui.label(RichText::new("👤 Private Chat").size(18.0).strong().color(Color32::from_gray(200)));
        ui.add_space(8.0);
        ...
        // - GROUPS SECTION ---
        ui.label(RichText::new("👥 Groups").size(18.0).strong().color(Color32::from_gray(200)));
        ui.add_space(8.0);
            ...
            .show(ui, |ui| {
                egui::ScrollArea::vertical().id_source("groups").max_height(ui.available_height() - 50.0).show(ui, |ui| {
                    if app.groups.is_empty() {
                        ui.label(RichText::new("You are not in any group.").italics().weak());
                    }
