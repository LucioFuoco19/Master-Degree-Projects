    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
            ...
            ui.horizontal_centered(|ui| {
                ...
                ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                    ...
                    if let Some(username) = username_display {
                        ...
                        ui.label(format!("Connected as: {}", username));
                    }
                });
            });
        });
        ...
        for result in events {
            match result {
                    ...
                    let should_add = match self.view {
                        View::PrivateChat(idx) => {
                            if let Some(chat) = self.private_chats.get(idx) {
                                // *** FIX APPLIED HERE ***
                                // Let's check if the message belongs to this chat by checking:
                                // 1. If the chat ID matches (as before)
                                // 2. OR if the sender is the user we are talking to (fixes bugs incoming messages)
                ...
                AppResult::RegisterSuccess => { self.is_loading = false; self.nav(View::Home); self.status = "Registered! Please login.".into(); }
                ...
                AppResult::Error { message } => { self.is_loading = false; self.status = format!("Error: {}", message); }
            }
        }
