...
impl TestClient {
    pub async fn connect_and_login(port: u16, username: String, password: &str) -> Result<Self> {
        ...
        // 1. Retry Connection: If the server is full, try again for a few seconds
        ...
        // 3. Accedi con "Smart Retry" (Cruciale per evitare errori DB sotto carico)
        let mut attempts = 0;
        let (token, user_id) = loop {
            ...
            match Self::recv_msg(&mut framed).await? {
                ...
                ServerMsg::Error { message } => {
                    attempts += 1;
                    // If the error is related to credentials (DB transaction not yet committed) or lock
                    if (message.contains("Credentials") || message.contains("locked")) && attempts < 5 {
                        sleep(Duration::from_millis(200)).await; // Wait & re-try
                        continue;
                    }
                    return Err(anyhow!("Login failed definitively: {}", message));
                }
                _ => return Err(anyhow!("Unexpected response post-login")),
            }
        };
...
async fn monitor_process(pid: u32, run_flag: Arc<AtomicBool>) -> (f32, u64) {
    ...
    while run_flag.load(Ordering::Relaxed) {
        ...
        // Less frequent refresh to save CPU
...
async fn run_scenario(test_number: usize, n_users: usize, duration_secs: u64, interval_ms: u64) -> Result<()> {
    ...
    for i in 0..n_users {
        ...
        // - CRUCIAL RAMP-UP ---
        // 5ms pause prevents 100 users from connecting at the same time,
        // preventing DB (SQLite Busy) and network blocking.
        if n_users >= 50 { sleep(Duration::from_millis(5)).await; }

        match TestClient::connect_and_login(port, name, "pass").await {
            Ok(c) => clients.push(c),
            Err(e) => eprintln!("Fatal error connecting client {}: {}", i, e),
        }
    }
    ...
    for i in 0..n_active {
        ...
        // Attempting to create chat with a minimum tolerance
        ...
        handles.push(tokio::spawn(async move {
            ...
            while Instant::now() < deadline {
                ...
                if client.send(ClientMsg::SendPrivateMessage { token: client.token, chat_id, content }).await.is_ok() {
                    // Tolerant timeout to avoid aborting the thread if the server lags for a moment
    ...
    eprintln!("=== FINISH TEST {} -> Msgs: {}/{}, CPU: {:.1}%, RAM: {}MB ===", 
...
async fn sequential_load_tests() -> Result<()> {
    ...
    // For test 5+ remember ulimit -n 2048 if you are on Mac
    run_scenario(5, 150, 60, 1000).await?;
    
    // Optional Test 6 (250 users) - only if you have powerful hardware
