...
use std::io::Write;
use std::path::Path; // Added to manage routes intelligently
...
impl TestClient {
    pub async fn connect_and_login(port: u16, username: String, password: &str) -> Result<Self> {
        ...
        let _ = Self::recv_msg(&mut framed).await.and_then(|m| match m {
            ...
            _ => Err(anyhow!("Unexpected post-registration response")),
        })?;
        ...
        let (token, user_id) = Self::recv_msg(&mut framed).await.and_then(|m| match m {
            ...
            _ => Err(anyhow!("Unexpected post-login response")),
        })?;
    ...
    async fn recv_msg(framed: &mut Framed<TcpStream, LengthDelimitedCodec>) -> Result<ServerMsg> {
        let packet = timeout(Duration::from_secs(10), framed.next()).await
            .map_err(|_| anyhow!("msg Timeout reception"))?
    ...
    pub async fn recv_until<T>(
        ...
        loop {
            ...
            if remain.is_zero() {
                return Err(anyhow!("recv_until Timeout expired"));
            }
...
async fn start_server_and_wait_with_env(envs: &[(&str, &str)]) -> Result<(std::process::Child, u16)> {
    ...
    let mut child = cmd
        ...
        .spawn()
        .expect("Unable to start server. Do 'cargo build --bin server' first.");
    ...
    let _ = child.kill();
    Err(anyhow!("The server did not start on {}", addr))
}
...
/// Function to write results to log file
/// Dynamically detect if we are in ROOT or SERVER to position the file correctly in:
...
fn log_test_result(scenario_label: &str, test_type: &str, details: &str, result_summary: &str) {
    ...
    // Dynamic logic to understand the basic path
    let base_path = if Path::new("server/tests").exists() {
        // We are in the root (G65)
        "server/tests/results"
    } else if Path::new("tests").exists() {
        // We are inside server (G65/server)
        "tests/results"
    } else {
        // Fallback: Are we already in tests? (Rare but possible)
        "results"
    };
    ...
    if let Err(e) = fs::create_dir_all(dir_path) {
        eprintln!("[LOG ERROR] Unable to create directory {:?}: {}", dir_path, e);
        return; 
    }
    ...
    match OpenOptions::new().create(true).append(true).open(&file_path) {
        Ok(mut file) => {
            if let Err(e) = file.write_all(log_entry.as_bytes()) {
                eprintln!("[LOG ERROR] Writing to file failed: {}", e);
            }
        },
        Err(e) => eprintln!("[LOG ERROR] Opening file failed: {}", e),
    }
}
...
async fn e2e_group_debug() -> Result<()> {
    ...
    eprintln!("[SETUP] Group created: {}", group_id);
    ...
    eprintln!("[SETUP] Bob added to group");
    ...
    eprintln!("[DONE] Send {} messagges", counter);

    // LOGGING
    log_test_result(
        "A Scenario",
        "Group Debug (Ping-Pong)",
        &format!("Duration: {}s, Rate: {}ms", duration_secs, rate_ms),
        &format!("OK - Exchanged {} messages", counter)
    );
...
async fn e2e_true_async_burst() -> Result<()> {
    ...
    let burst_size = 100; // Send 100 messagges "in bursts"
    eprintln!("[TRUE BURST] Sending {} messages without waiting for ACK...", burst_size);

    // 1. SEND EVERYTHING TOGETHER (Flood)
    ...
    // 2. NOW COLLECT THE ACKS
    eprintln!("[TRUE BURST] Waiting ACK...");
        ...
        loop {
            ...
            alice.recv_until(Duration::from_secs(5), |m| match m {
                ServerMsg::GroupMessageSent { .. } => {
                    acks += 1;
                    if acks % 20 == 0 { eprintln!("   ...received {}/{}", acks, burst_size); }
                    Some(())
                },
                _ => None
            }).await?;
    ...
    timeout(Duration::from_secs(10), receive_loop).await
        .map_err(|_| anyhow!("Timeout! Received only {}/{} ACK", acks, burst_size))??;

    let elapsed = start.elapsed();
    eprintln!("[TRUE BURST] Successo! {} msg processed in {:.2?}", burst_size, elapsed);
    ...
    log_test_result(
        ...
        &format!("OK - Processed in {:.2?}", elapsed)
    );
...
async fn e2e_read_write_pressure() -> Result<()> {
    ...
    eprintln!("[RW PRESSURE] Survived concurrent reads and writes.");
