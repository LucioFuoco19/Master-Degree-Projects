...
impl SqliteStorage {
    pub fn new(db_path: &str) -> AppResult<Self> {
        ...
        // =================================================================
        // BALANCED CONFIGURATION (Performance vs. Consistency)
        // =================================================================
        
        // 1. WAL Mode: Required to support 100+ users without continuous blocking.
        conn.pragma_update(None, "journal_mode", "WAL")?;

        // 2. Timeout busy: If the DB is busy, wait 5s (CRUCIAL to avoid "database is locked").
        conn.pragma_update(None, "busy_timeout", 5000)?;

        // 3. Synchronous: Returned to FULL (or NORMAL). 
        // 'NORMAL' caused the "Credentials Incorrect" race condition on fast machines.
        // 'FULL' ensures that after the Register, the user actually exists for Login.
    ...
    pub fn insert_session(&self, username: &str, ttl_secs: i64) -> AppResult<Uuid> {
        ...
        // We use now_unix() updated to milliseconds
        let now = now_unix();
        // Convert ttl to ms for consistency, or treat it as seconds added to ms
