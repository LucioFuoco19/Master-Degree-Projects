...
pub fn handle_search_users(db: &SqliteStorage, token: Uuid, query: String) -> AppResult<Vec<UserInfo>> {
    // *** EDIT: We get the user_id instead of ignoring it ***
...
pub fn handle_start_private_chat(db: &SqliteStorage, peers: &PeerMap, token: Uuid, other_username: String) -> AppResult<Uuid> {
    let user_id = db.validate_session(&token)?.ok_or(AppError::BadCredentials)?;
    let other_id = db.get_user_id(&other_username)?.ok_or(AppError::Validation("User not found".into()))?;
    if user_id == other_id { return Err(AppError::Validation("No chat with yourself".into())); }
...
pub fn handle_get_private_chat_messages(db: &SqliteStorage, token: Uuid, chat_id: Uuid, limit: u32) -> AppResult<Vec<MessageInfo>> {
    let user_id = db.validate_session(&token)?.ok_or(AppError::BadCredentials)?;
    if !db.is_user_in_private_chat(&user_id, &chat_id)? { return Err(AppError::Validation("Access denied".into())); }
...
pub fn handle_send_private_message(db: &SqliteStorage, peers: &PeerMap, token: Uuid, chat_id: Uuid, content: String) -> AppResult<Uuid> {
    let user_id = db.validate_session(&token)?.ok_or(AppError::BadCredentials)?;
    if content.trim().is_empty() { return Err(AppError::Validation("Empty message".into())); }
    if !db.is_user_in_private_chat(&user_id, &chat_id)? { return Err(AppError::Validation("Access denied".into())); }
...
pub fn handle_create_group(db: &SqliteStorage, token: Uuid, name: String) -> AppResult<Uuid> {
    let user_id = db.validate_session(&token)?.ok_or(AppError::BadCredentials)?;
    if name.trim().is_empty() { return Err(AppError::Validation("Empty name".into())); }
...
pub fn handle_add_group_member(db: &SqliteStorage, peers: &PeerMap, token: Uuid, group_id: Uuid, username: String) -> AppResult<()> {
    let user_id = db.validate_session(&token)?.ok_or(AppError::BadCredentials)?;
    if !db.is_user_in_group(&user_id, &group_id)? { return Err(AppError::Validation("Not a member".into())); }
    let target = db.get_user_id(&username)?.ok_or(AppError::Validation("User not found".into()))?;
    if target == user_id { return Err(AppError::Validation("Self-adding forbidden".into())); }
    if db.is_user_in_group(&target, &group_id)? { return Err(AppError::Validation("Already member".into())); }
...
pub fn handle_get_group_members(db: &SqliteStorage, token: Uuid, group_id: Uuid) -> AppResult<Vec<UserInfo>> {
    let user_id = db.validate_session(&token)?.ok_or(AppError::BadCredentials)?;
    if !db.is_user_in_group(&user_id, &group_id)? { return Err(AppError::Validation("Access denied".into())); }
