...
pub async fn serve_connection(sock: TcpStream, db_path: &'static str, peers: PeerMap) {
                ...
                Some(Ok(bytes)) => {
                    ...
                    if let ClientMsg::Listen { token } = &msg {
                         match db.validate_session(token) {
                            Ok(Some(user_id)) => {
                                println!("User {} listening push", user_id);
                                peers.lock().unwrap().insert(user_id, tx_push.clone());
                                my_user_id = Some(user_id);
                            }
                            _ => {
                                let _ = send(&mut framed, &ServerMsg::Error { message: "Invalid session".into() }).await;
                                break;
                            }
                         }
                         continue;
    ...
    // *** BUG FIX: SECURE DISCONNECT ***
    if let Some(uid) = my_user_id {
        let mut peers_guard = peers.lock().unwrap();

        // Let's check if there is still a channel for this user
        if let Some(stored_tx) = peers_guard.get(&uid) {
            // We remove the user from the map ONLY IF the stored channel
            // that's exactly what THIS connection is closing.
            // If it's different, it means the user has reconnected elsewhere
            // and we don't have to delete his new session.
            if stored_tx.same_channel(&tx_push) {
                peers_guard.remove(&uid);
                println!("User {} disconnected (session clean)", uid);
            } else {
                println!("User {} disconnected (session preserved, new connection active)", uid);
            }
        }
    }
}
...
fn handle_client_msg(msg: &ClientMsg, db: &SqliteStorage, peers: &PeerMap) -> ServerMsg {
    match msg {
        ...
        _ => ServerMsg::Error { message: "Unmanaged message".into() }
    }
}
...
fn map_error(e: AppError) -> ServerMsg {
    use AppError::*;
    let message = match e {
        UserExists => "Username already in use".into(),
        BadCredentials => "Bad credentials".into(),
        Validation(m) => m,
